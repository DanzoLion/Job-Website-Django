...
class MyAdminAccounts(UserAdmin):
    ...
    readonly_fields = ['date_joined']                                                   # we need this field as django add this as a default in our table fields
...
admin.site.register(Profile)                                                            # we can now display this model in our admin panel
admin.site.register(Invite)                                                             # we can now display this model in our admin panel, then makemigrations -> tables & fields added to database